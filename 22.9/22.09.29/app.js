...
// 라이브러리라는건 즉 없어도 리액트로 작업물을 만들수 있지만 편하게 작업하려고 쓰는것

// 실무에서 리덕스를 안쓰면 매우 불편하기 때문에 필수로 사용한다.

// 리액트는 자식 컴포넌트에 props로 전달이 가능한데
// 다른 컴포넌트에서 직접 데이터 공유가 불가능하다..
// 그래서 다른 공유해야할 데이터를 공유받는 자식 컴포넌트들의 공통부모에서 State를 만들어서
...
// 근데 초반에 리덕스를 처음보는 사람들이 많이 개념을 어려워 한다.
// 쉽게 생각해서 컴포넌트에서 값을 요청하고 전달받는걸 직접 할수 있다.
...
// 컴포넌트가 Action을 보내고 Reducer로 전달 받고 store의 값을 최신화 해준다.
// Action은 동작한 기능의 이름
...
// redux는 리액트에서만 쓸수 있는게 아니라 리액트도 사용할수 있는것
...
// Provider
// Provider 컴포넌트이고 이 컴포넌트로 감싸준 컴포넌트 리덕스 저장소에 사용을 가능하게 만들어 준다.
