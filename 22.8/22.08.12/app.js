...
// Access Token만 쓰는 방식은

// 1.이용자가 로그인 시도를 하고
// 2.서버에서 이용자를 확인하고 입장권을 발급하는데
// JWT토큰 인증 정보를 payload에 할당하고 생성
// 3.생성한 토큰을 클라이언트에 반환해주고 클라이언트가 이 토큰을 가지고 있다.
// 4.클라이언트에서 권한을 인증 요청할때 이 토큰을 같이 보낸다.
// 5.서버는 토큰을 확인하고 payload의 값 인코딩되어 있는 값을 디코딩해서
// 사용자의 권한을 확인하고 (입장권이 맞는지) 데이터를 반환한다.
// 6.만약에 토큰이 정상적이지 확인하고 (토큰이 썩었는지 입장권 시간이 지났는지)
// 7.날짜가 지난 토큰이면 새로 로그인 시킨다. 토큰 재발급(입장권 새로 산다.)

// Refresh Token와 같이 사용하면 
// Access token 만쓰면 인증보안에 취약할수 있고 다른사람이 악의적으로 토큰을
// 취득하면 토큰의 유효시간이 끝나기 전까지는 막을수가 없다.(이미 입장권 보여주고 입장함)
// Access token의 유효기간을 짧게하고 유효기간이 짧으니까 로그인을 자주
// 해야하는 문제가 생기는데 (사용자가 이용하기 힘들다.) Refresh token으로 해결할수 있다.
// Refresh token과 Access token 은 둘다 JWT토큰이고
// Refresh token은 유효기간을 길게 주고 Access token이 유효시간이 끝났을때
// 발급해주는 역활만 한다.

// 보통 Access token을 30분만 주면
// Refresh token의 유효시간을 하루 주고
// Access token의 유효시간이 끝나면 Refresh token의 유효기간을 확인하고
// Access token을 재발급 해준다.

// Access token과 Refresh token 둘다 이용한 인증 방식
// 1.이용자가 로그인 
// 2.서버에서 사용자를 확인하고 입장권 권한 인증 정보를 payload에 할당해서 생성하고
// Refresh token도 생성해서 서버에 저장하고 두토큰 모두 클라이언트에 반환한다.
// 3.클라이언트도 두 토큰을 저장한다.
// 4.클라이언트가 권한이 인증이 필요해서 요청하면 Access token을 전달해서 요청한다.
// 5.서버는 전달받은 토큰을 확인하고 payload의 인코딩된 값을 디코딩해서 사용자의 권한을 
// 확인한다.
// 6.만약에 토큰이 정상적인지 확인을 하고(썩은 토큰인지)
// 7.날짜가 지난 토큰이면 새로 로그인 시켜서 토큰을 발급 받게 한다.
// (만료된 Access token 과 Refresh token을 해더에 실어서 서버에게 보낸다.)
// 8.서버는 Access token 과 Refresh token을 확인하고 Refresh token이 
...
app.post("/login",(req,res)=>{
    ...
    if(user_id === user.id&& password === user.password){
        ...
        // fs 에 readFile함수를 사용해서 join.html을 불러온다 . res의 send 함수로 응답
...
// 막기가 힘들어서 access token의 유효기간을 짧게하고 feresh token의 유효기간을 
// 길게해서 사용자가 로그인을 자주 하는 불편함을 보안해 준다.
