...
import { DIFFICULTY_ADJUSTMENT_INTERVAL,
        ...
        BLOCK_GENERATION_TIME_UNIT} from "@core/config";
...
export class Block extends BlockHeader implements IBlock {
    ...
    public static getDifficulty(_newBlock : Block, _adjustmentBlock : Block , _previousBlock : Block) : number{
        ...
        const timeTakoen : number = _newBlock.timestamp - _adjustmentBlock.timestamp;
        const TimeExpected : number = 
        BLOCK_GENERATION_INTERVAL *
        BLOCK_GENERATION_TIME_UNIT *
        DIFFICULTY_ADJUSTMENT_INTERVAL; // 6000

        if(timeTakoen < TimeExpected / 2) 
        return _adjustmentBlock.difficulty + 1;
        else if(timeTakoen > TimeExpected * 2) 
        return _adjustmentBlock.difficulty - 1;
    ...
    public static createBlockHash(_block : Block) : string{
        ...
        const values : string = `${version}${timestamp}${height}${merkleRoot}${previousHash}${nonce}${difficulty}`
