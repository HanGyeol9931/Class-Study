...
// 비밀키랑 공개키와 서명을 이용한 신원 인증 방식은 분산원장이라는 이해가 필요한데

// 분산 원장 : 복제 공유 동기화된 데이터에 대한 합의 기술

// 금융 쪽에서는 장부를 가지고 모든 거래내역을 기록하는 방식
// 은행이라는 금융 기관은 장부에 거래를 의존할수 밖에 없는 중앙 집권화된 방식
...
// 대칭형 : 암호화 복호화 할떄 사용하는 키가 동일한 경우 1개를 사용한다.
// 암호화한 사람과 수신하는사람 또한 같은 키를 같은 키를 가지고 있어야 한다.

// 비대칭형 : 사용하는 키와 복호화 할때 사용하는 키가 다른 것 다른사람에게 절대
// 공개되서는 안되느 키(비밀키) 비밀키를 토대로 만든 공개키가 한쌍이다.
// 결록은 키를 2개를 사용한다는것
...
describe("지갑 만들기",()=>{
    ...
    // 공개를 생성한 소유자에게 의해 만들어진 서명인게 증명된다.
    it("검증하기",()=>{
        // 필요한 값 : 서명, 공개키 , hash
        const hash = SHA256("transaction data").toString();
    ...
    it("지갑 주소",()=>{
        ...
        // 버퍼는 일반적으로 PC에 할당된 메모리 청크 RAM을 나타내고 버퍼의 크기를 서러정하면
        // 변경할수 없다.
        // 데이터를 담아놓고 사용할 시기를 제어할수 있는
        // nodejs에서 데이터를 전송하는 속도가은 부분은 제어하기 위해서 사용한다.
        
        // Buffer.from 은 버퍼를 만드는 방법 string 배열을 인수로 받고 
        // 넘겨주는 값에 따라 약간씩 다른 방법으로 만들어 준다.
        const buffer = Buffer.from(pubKey);
        // 이더리움 방식으로 계정을 만드는 방법은 만들어 놓은 공개키에서 앞의
        // 24자리를 잘라내고 40자리만큼을 남겨주면 된다.
        // slice(26)을 넣은 이유가 elliptic를 이용해서 만든 공개키 앞자리
        // 02혹은 03이 붙어서 이값도 제거하기 위함
    ...
    // 타원 곡선 알고리즘을 직접 구현한ㄴ건 너무 힘드니까 개인키를 사용해서 공개키를
