...
export class Wallet {
    ...
    constructor(privateKey : string=""){
        ...
        // 전달된 개인키가 있는지
        // 지갑이 생성 되어 있는 지갑인지
        // 지갑이 없고 새로 생성할때 getPribatKey 함수를 통해서 개인키를 생성해준다
        ...
        this.balance = 0;
        // fs 모듈을 사용해서 프로그램을 통해서 지갑을 만들때 개인키를 안전하게 저장하는게
        // 중요한 이유고 그래서 루트 폴더에 data 폴더를 만들어 준후
        // createWallet 함수를 사용할때 마다 data폴더에
        // 계정명과 파일명을 가지고 privateKey 값을 내용을 파일로 생성해줬다.
    ...
    public getAccount(): string{
        // Buffer를 사용한 이유는 바이너리 데이터를 조작하기 위해서
        // Buffer에 있는 동안 바이너리 데이터를 조작할수 있기 떄문에.
