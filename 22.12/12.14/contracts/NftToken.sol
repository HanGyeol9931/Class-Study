...
contract NftToken is ERC721Enumerable, Ownable{
    ...
    // NFT 발행량을 제한 할 상태 변수
    // 상수를 사용 할건데 솔리디티에서 상수는 constant를 타입에 붙여서 선언
    ...
    function getRandomTokenData(address _owner, uint _tokenId) private pure returns (TokenData memory){
        ...
        // keccak256 함수가 해주는 역활은 32바이트로 변환 해준다.
        // 솔리디티에서 랜덤값을 구하는 방법
    ...
    // TokenData의 Type 조회하는 함수
    function getTokenType(uint _tokenId) public view returns (uint){
        return TokenDatas[_tokenId].Type;    
    }
    // 배열 전체 내용을 조회하는 함수
    // 솔리디티에서 배열을 getter로 전체 조회하는 불가능한데 요소를 하나만 return 하기 때문에
    // 배열 전체를 return 해주는 view 함수를 만들어 주면 된다.
