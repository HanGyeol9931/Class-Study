...
contract ERC20 {
    ...
    // 양도 했다는 뜻이다. 2번 계정도 3번 계정에게 200의 토큰의 권한을 위임 하기위해서
    // 2번 계정이 200 토큰 이상이 넘지 않는 선에서 
    // 3번 계정의 토큰을 다른 사람에게 전송할수 있다.

    // 잔액을 확인하는 함수
    // external은 오직 다른곳에서 불러서 사용가능 접근 가능
    // 정의된 컨트랙트 안에서는 사용할수 없다.(여기서 사용 불가)
    ...
    function transferFrom(address sender, address recipient, uint amount) external returns (bool){
        ...
        // msg.sender transferFrom 함수를 호출한 계정으로 특정 소유자로 부터 일정량의 토큰을 위임받은 계정
    }

    // internal : 정의된 컨트랙트 내에서 또는 상속받은 자식 컨트랙트에서만 접근 가능
    // private과 비슷 하지만 상속받은 자식도 접근 가능하다는 점이 다르다.

    // 발행량을 늘려주는 함수 CA로 전송한 계정이 발행자일 경우에 사용할것.
    function mint(uint amount) internal {
        balances[msg.sender] += amount;
        totalSupply += amount;
        // address(0) from 값이 필요없기 때문에 null로 넣어준것
